...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "開啟新對話（全新 Session ID 與歷史紀錄）", "Session",
               aliases=("reset",)),
    CommandDef("clear", "清空畫面並開啟新對話", "Session",
               cli_only=True),
    CommandDef("redraw", "強制重繪 UI（修復終端機跑版）", "Session",
               cli_only=True),
    CommandDef("history", "顯示對話紀錄", "Session",
               cli_only=True),
    CommandDef("save", "儲存目前對話", "Session",
               cli_only=True),
    CommandDef("retry", "重試上一則訊息（重新發送給 Agent）", "Session"),
    CommandDef("undo", "移除最後一次對話往來", "Session"),
    CommandDef("title", "設定目前對話的標題", "Session",
               args_hint="[name]"),
    CommandDef("branch", "分支目前對話（探索不同路線）", "Session",
               aliases=("fork",), args_hint="[name]"),
    CommandDef("compress", "手動壓縮對話上下文", "Session",
               args_hint="[focus topic]"),
    CommandDef("rollback", "列出或還原檔案系統檢查點", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "建立或還原 Hermes 設定/狀態快照", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "強制終止所有背景程序", "Session"),
    CommandDef("approve", "核准等待中的危險指令", "Session",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "拒絕等待中的危險指令", "Session",
               gateway_only=True),
    CommandDef("background", "在背景執行提示詞", "Session",
               aliases=("bg", "btw"), args_hint="<prompt>"),
    CommandDef("agents", "顯示使用中 Agent 與執行中的任務", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "將提示詞排入下回合（不中斷目前執行）", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下次呼叫工具後注入訊息（不中斷執行）", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "設定 Hermes 持續執行的目標，直到達成", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("status", "顯示對話資訊", "Session"),
    CommandDef("profile", "顯示目前的 Profile 名稱與家目錄", "Info"),
    CommandDef("sethome", "將此對話設為首頁頻道", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "繼續之前命名的對話", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("config", "顯示目前設定", "Configuration",
               cli_only=True),
    CommandDef("model", "切換此對話使用的模型", "Configuration",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("gquota", "顯示 Google Gemini Code Assist 額度使用量", "Info",
               cli_only=True),

    CommandDef("personality", "設定預設的人格特質", "Configuration",
               args_hint="[name]"),
    CommandDef("statusbar", "切換上下文/模型狀態列", "Configuration",
               cli_only=True, aliases=("sb",)),
    CommandDef("verbose", "切換工具進度顯示模式：關閉 -> 新增 -> 全部 -> 詳細",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "切換最終回覆的 Gateway 執行中繼資料頁尾",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "切換 YOLO 模式（跳過所有危險指令確認）",
               "Configuration"),
    CommandDef("reasoning", "管理推論強度與顯示方式", "Configuration",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "切換快速模式 — OpenAI 優先處理 / Anthropic 快速模式", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "顯示或變更顯示面板/主題", "Configuration",
               cli_only=True, args_hint="[name]"),
    CommandDef("indicator", "選擇 TUI 忙碌指示器風格", "Configuration",
               cli_only=True, args_hint="[kaomoji|emoji|unicode|ascii]",
               subcommands=("kaomoji", "emoji", "unicode", "ascii")),
    CommandDef("voice", "切換語音模式", "Configuration",
               args_hint="[on|off|tts|status]", subcommands=("on", "off", "tts", "status")),
    CommandDef("busy", "控制 Hermes 運作時按 Enter 的行為", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "管理工具：/tools [list|disable|enable] [名稱...]", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "列出可用的工具集", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "搜尋、安裝、檢查或管理技能",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "管理排程任務", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "背景技能維護（狀態、執行、釘選、封存）",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore")),
    CommandDef("kanban", "多重 Profile 協作看板（任務、連結、評論）",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload", "重新載入 .env 變數到執行中的對話", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "從設定檔重新載入 MCP 伺服器", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "重新掃描 ~/.hermes/skills/ 以尋找新安裝或移除的技能",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "透過 CDP 將瀏覽器工具連接至目前的 Chrome", "Tools & Skills",
               cli_only=True, args_hint="[connect|disconnect|status]",
               subcommands=("connect", "disconnect", "status")),
    CommandDef("plugins", "列出已安裝的套件與狀態",
               "Tools & Skills", cli_only=True),

    # Info
    CommandDef("commands", "瀏覽所有指令與技能（分頁）", "Info",
               gateway_only=True, args_hint="[page]"),
    CommandDef("help", "顯示可用指令", "Info"),
    CommandDef("restart", "在目前任務執行完畢後，優雅地重啟 Gateway", "Session",
               gateway_only=True),
    CommandDef("usage", "顯示目前對話的 Token 使用量與速率限制", "Info"),
    CommandDef("insights", "顯示使用量洞察與分析", "Info",
               args_hint="[days]"),
    CommandDef("platforms", "顯示 Gateway/通訊平台狀態", "Info",
               cli_only=True, aliases=("gateway",)),
    CommandDef("copy", "複製上一則回覆到剪貼簿", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "附加剪貼簿中的圖片", "Info",
               cli_only=True),
    CommandDef("image", "附加本機圖片檔案到下個提示詞", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "更新 Hermes Agent 到最新版本", "Info",
               gateway_only=True),
    CommandDef("debug", "上傳除錯報告（系統資訊與日誌）並取得分享連結", "Info"),

    # Exit
    CommandDef("quit", "離開命令列介面", "Exit",
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("hermes", "與 Hermes 對話或執行子指令", "[subcommand] [args]"))
    seen.add("hermes")
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        for alias in cmd.aliases:
            ...
            _add(alias, f"為 /{cmd.name} 的別名 — {cmd.description}", cmd.args_hint or "")
